...
class TrainInterface(object):
    ...
    def __train(model, train_loader, optimizer, epoch, num_train, opts):
        ...
        for i,(imgs, labels) in enumerate(train_loader):
            labels1 = labels.view(opts.batch_size, GL_NUMGRID, GL_NUMGRID, -1)
            labels1 = labels1.permute(0,3,1,2)
            if opts.use_GPU:
                imgs = imgs.to(device)
                labels1 = labels1.to(device)
            preds = model(imgs)  # 前向传播
            loss = model.calculate_loss(preds,labels1)  # 计算损失
        ...
        log_file.write("Epoch %d/%d | training loss = %.3f, avg_loss = %.3f\n" %
                  (epoch, opts.epoch, loss.item(), losssum/num_train))
        log_file.flush()
        log_file.close()
    ...
    def main(self):
        ...
        best_metric=1000000
        for e in range(opts.start_epoch+60, opts.epoch+31):
            t = time.time()
